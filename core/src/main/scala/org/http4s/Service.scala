...
object Service {
  ...
  /** Lifts a partial function to an `Service`.  Responds with the
    ...
    */
  def apply[F[_]: Applicative, A, B: Monoid](pf: PartialFunction[A, F[B]]): Service[F, A, B] =
    lift(req => pf.applyOrElse(req, Function.const(Applicative[F].pure(Monoid[B].empty))))
