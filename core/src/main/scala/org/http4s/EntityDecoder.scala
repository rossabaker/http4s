...
trait EntityDecoder[F[_], T] { self =>
  ...
  def map[T2](f: T => T2)(implicit F: Functor[F]): EntityDecoder[F, T2] = new EntityDecoder[F, T2] {
    ...
    override def decode(msg: Message[F], strict: Boolean): DecodeResult[F, T2] = self.decode(msg, strict).map(f)
  }

  def flatMapR[T2](f: T => DecodeResult[F, T2])(implicit F: Monad[F]): EntityDecoder[F, T2] = new EntityDecoder[F, T2] {
    override def decode(msg: Message[F], strict: Boolean): DecodeResult[F, T2] = self.decode(msg, strict).flatMap(f)
...
object EntityDecoder extends EntityDecoderInstances {
  ...
  /** Create a new [[EntityDecoder]]
    ...
    */
  def decodeBy[F[_]: Applicative, T](r1: MediaRange, rs: MediaRange*)(f: Message[F] => DecodeResult[F, T]): EntityDecoder[F, T] = new EntityDecoder[F, T] {
  ...
  private class OrDec[F[_]: Functor, T](a: EntityDecoder[F, T], b: EntityDecoder[F, T]) extends EntityDecoder[F, T] {
    override def decode(msg: Message[F], strict: Boolean): DecodeResult[F, T] = {
      msg.headers.get(`Content-Type`) match {
  ...
  def collectBinary[F[_]](msg: Message[F])(implicit F: Catchable[F]): DecodeResult[F, Chunk[Byte]] =
    DecodeResult.success(msg.body.chunks.runFoldMap[Chunk[Byte]](identity))

  /** Decodes a message to a String */
  def decodeString[F[_]](msg: Message[F])(implicit F: Catchable[F], defaultCharset: Charset = DefaultCharset): F[String] =
...
trait EntityDecoderInstances {
  ...
  def error[F[_]: Catchable, T](t: Throwable): EntityDecoder[F, T] = new EntityDecoder[F, T] {
    override def decode(msg: Message[F], strict: Boolean): DecodeResult[F, T] = {
      DecodeResult(msg.body.open.close.run.flatMap(_ => Catchable[F].fail[Either[DecodeFailure, T]](t)))
    }
    override def consumes: Set[MediaRange] = Set.empty
  }

  implicit def binary[F[_]: Catchable]: EntityDecoder[F, Chunk[Byte]] = {
    EntityDecoder.decodeBy[F, Chunk[Byte]](MediaRange.`*/*`)(collectBinary)
  }

  implicit def text[F[_]: Catchable](implicit defaultCharset: Charset = DefaultCharset): EntityDecoder[F, String] =
  ...
  def binFile[F[_]: Catchable: Suspendable](file: File): EntityDecoder[F, File] =
    EntityDecoder.decodeBy(MediaRange.`*/*`){ msg =>
      val sink = writeOutputStream[F](Suspendable[F].delay(new FileOutputStream(file)))
      DecodeResult.success(msg.body.to(sink).run).map(_ => file)
    }

  def textFile[F[_]: Catchable: Suspendable](file: File): EntityDecoder[F, File] =
    EntityDecoder.decodeBy(MediaRange.`text/*`){ msg =>
      val sink = writeOutputStream[F](Suspendable[F].delay(new PrintStream(new FileOutputStream(file))))
      DecodeResult.success(msg.body.to(sink).run).map(_ => file)
    }
...
*/
  ...
  implicit def void[F[_]: Catchable]: EntityDecoder[F, Unit] =
    EntityDecoder.decodeBy(MediaRange.`*/*`)(msg =>
