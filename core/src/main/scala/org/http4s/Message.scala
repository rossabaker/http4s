...
sealed trait Message[F[_]] extends MessageOps[F] { self =>
  ...
  /** Decode the [[Message]] to the specified type
    ...
    */
  override def attemptAs[T](implicit decoder: EntityDecoder[F, T]): DecodeResult[F, T] =
...
final case class Request[F[_]](
  ...
  def decodeWith[A](decoder: EntityDecoder[F, A], strict: Boolean)(f: A => F[Response[F]])(implicit FF: Functor[F], FFM: FlatMap[F], EE: EntityEncoder[F, String]): F[Response[F]] =
...
case object Pass extends MaybeResponse[Nothing] {
  def now[F[_]: Applicative]: F[MaybeResponse[F]] =
    Applicative[F].pure(Pass)
}
